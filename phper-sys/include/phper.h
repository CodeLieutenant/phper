...
// ==================================================
 ...
 zval *phper_zend_hash_str_update(HashTable *ht, const char *key,
                                               size_t len, zval *pData);

 zval *phper_zend_hash_index_update(HashTable *ht, zend_ulong h,
                                                 zval *pData);

 zval *phper_zend_hash_next_index_insert(HashTable *ht,
                                                      zval *pData);

 void phper_array_init(zval *arg);
 void *phper_zend_hash_str_find_ptr(const HashTable *ht,
                                                 const char *str, size_t len);
 bool phper_zend_hash_str_exists(const HashTable *ht,
                                              const char *str, size_t len);
 bool phper_zend_hash_index_exists(const HashTable *ht,
                                                zend_ulong h);
 zend_array *phper_zend_new_array(uint32_t size);
 zend_array *phper_zend_array_dup(zend_array *source);
 zval *phper_zend_hash_index_find(const HashTable *ht,
                                               zend_ulong h);
 bool phper_zend_hash_index_del(HashTable *ht, zend_ulong h);
 zval *phper_zend_symtable_str_update(HashTable *ht,
                                                   const char *str, size_t len,
                                                   zval *pData);

 bool phper_zend_symtable_str_del(HashTable *ht, const char *str,
                                               size_t len);
 zval *phper_zend_symtable_str_find(HashTable *ht, const char *str,
                                                 size_t len);

 bool phper_zend_symtable_str_exists(HashTable *ht,
                                                  const char *str, size_t len);

 zval *phper_zend_str_update(HashTable *ht, const char *str,
                                          size_t len, zval *pData);
 bool phper_zend_str_del(HashTable *ht, const char *str,
                                      size_t len);
 zval *phper_zend_str_find(HashTable *ht, const char *str,
                                        size_t len);
 bool phper_zend_str_exists(HashTable *ht, const char *str,
                                         size_t len);
